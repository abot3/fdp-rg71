...
mid_atlantic_data = car_data[car_data$region == "Mid Atlantic",]
drops = c("region")
mid_atlantic_data = mid_atlantic_data[ , !(names(mid_atlantic_data) %in% drops)]

# fit base model
base_atlantic_model = lm(price ~ engine_displacement
                         ...
                         + salvage + franchise_dealer, data = mid_atlantic_data)
head(summary(base_atlantic_model)$coefficients, 10)
length(coef(base_atlantic_model))
calc_loocv_rmse(base_atlantic_model)
cal_rmse(base_atlantic_model)

# fit base model + state (state is significant)
base_atlantic_model_state = lm(price ~ engine_displacement
                               + engine_cylinders + highway_fuel_economy + horsepower
                               + maximum_seating + transmission + mileage + year
                               + wheel_system + power_hp + bed_length + #cabin
                               + salvage + franchise_dealer + state,
                               data = mid_atlantic_data)
head(summary(base_atlantic_model_state)$coefficients, 10)
length(coef(base_atlantic_model_state))
calc_loocv_rmse(base_atlantic_model_state)
cal_rmse(base_atlantic_model_state)
anova(base_atlantic_model, base_atlantic_model_state)

# fit full mid atlantic model
full_midat_model = lm(price ~ ., data = mid_atlantic_data)
...
# refit model with identified significant interactions
significant_int_predictors = c("year:bed_length", "year:mileage", "maximum_seating:bed_length",
...
<!-- ### v. Global Model Diagnostic Visualizations -->
```{r ANOVA Analysis of Global Model, results = 'hide', include = FALSE}
...
# improved_mid_atlantic_cox = lm(formula = (price ^ 0.26 - 1)/0.26 ~ back_legroom
#                          + bed_length + daysonmarket + franchise_dealer
#                          + fuel_tank_volume + fuel_type + height
#                          + highway_fuel_economy + horsepower + isCab + is_new
#                          + longitude + maximum_seating + mileage + seller_rating
...
#                          + torque_rpm + state,
#                          data = mid_atlantic_data)
# summary(improved_mid_atlantic_cox)
# bptest(improved_mid_atlantic_cox)
# plot_fitted_resid(improved_mid_atlantic_cox)
# 
# improved_mid_atlantic_log = lm(log(price) ~ engine_displacement + engine_cylinders
#              + highway_fuel_economy + horsepower + maximum_seating + mileage
#              + year + wheel_system + bed_length + franchise_dealer,
#              data = mid_atlantic_data)
# summary(improved_mid_atlantic_log)
# bptest(improved_mid_atlantic_log)
# plot_fitted_resid(improved_mid_atlantic_log)
# 
# improved_mid_atlantic_log_reduced = lm(log(price) ~ engine_displacement + engine_cylinders
#                      + highway_fuel_economy + horsepower + maximum_seating
#                      + mileage + year + wheel_system + bed_length
#                      + franchise_dealer,
#                      data = mid_atlantic_data)
# summary(improved_mid_atlantic_log_reduced)
# bptest(improved_mid_atlantic_log_reduced)
# plot_fitted_resid(improved_mid_atlantic_log_reduced)
# 
# improved_mid_atlantic_log_int = lm(log(price) ~ engine_displacement + engine_cylinders
#                      + highway_fuel_economy + horsepower + maximum_seating
#                      + mileage + year + wheel_system + bed_length
...
#                      data = mid_atlantic_data)
# summary(improved_mid_atlantic_log_int)
# bptest(improved_mid_atlantic_log_int)
# plot_fitted_resid(improved_mid_atlantic_log_int)
...
Comparing a **fitted vs residuals** plot from before and after the log-transformation
of the response variable, as well as an analysis of the results from the
log model, we see very significant improvements.
...
```{r Significant Variable Transformations and Interactions pt. 2, results = 'hide', warning=FALSE}
...
# ANOVA test for regional vs global models
...
# diagnostics for regional model
...
```{r Model Comparison Table, echo = FALSE, include = FALSE}
base_model = comparison_stats(mid_atlantic_base_model)
full_modle = comparison_stats(mid_atlantic_full_model)
base_bic_model = comparison_stats(base_model_bic)
base_interaction_model = comparison_stats(mid_atlantic_int_model)
improved_interaction_model= comparison_stats(improved_mid_atlantic_int_model)
log_model = comparison_stats(log_mid_atlantic_int_model)
stats = rbind(base_model, full_modle, base_bic_model, base_interaction_model, improved_interaction_model, log_model)
knitr::kable(stats)
```

```{r}
# TODO
### We should show comparisons between these models and make our selection (our global model), including explanations as to why this decision was made.

### For the models that we choose not to proceed with, a brief explanation behind why they were not favorable will be sufficient.

### Should we plot anything with our commentary stating why it is not an adequate or best model selection?

### Do we want to move all of our commentary from the Methods section into Results?
```

##### Base Model (hypothesis)
...
### Let's be sure to talk about how we set our targets for the project and why we chose those, since there is expectation around us having explored the balancing of different measures that pertain to the "wellness of fit" of a model.
```

|Model                          |$(p - 1)$                                             |LOOCV RMSE                                           |Adjusted $R^2$                                             |
|:------------------------------|:-----------------------------------------------------|:----------------------------------------------------|:----------------------------------------------------------|
|**Base Model** (hypothesis)    |`r length(coef(mid_atlantic_base_model)) - 1`         |`r calc_loocv_rmse(mid_atlantic_base_model)`         |`r summary(mid_atlantic_base_model)$adj.r.squared`         |
|**Full Model**                 |`r length(coef(mid_atlantic_full_model)) - 1`         |`r calc_loocv_rmse(mid_atlantic_full_model)`         |`r summary(mid_atlantic_full_model)$adj.r.squared`         |
|**Base BIC Model**             |`r length(coef(base_model_bic)) - 1`                  |`r calc_loocv_rmse(base_model_bic)`                  |`r summary(base_model_bic)$adj.r.squared`                  |
|**Base Interaction Model**     |`r length(coef(mid_atlantic_int_model)) - 1`          |`r calc_loocv_rmse(mid_atlantic_int_model)`          |`r summary(mid_atlantic_int_model)$adj.r.squared`          |
|**Improved Interaction Model** |`r length(coef(improved_mid_atlantic_int_model)) - 1` |`r calc_loocv_rmse(improved_mid_atlantic_int_model)` |`r summary(improved_mid_atlantic_int_model)$adj.r.squared` |
|**Log Transform Model**        |`r length(coef(log_mid_atlantic_int_model)) - 1`      |`r calc_loocv_rmse(log_mid_atlantic_int_model)`      |`r summary(log_mid_atlantic_int_model)$adj.r.squared`      |

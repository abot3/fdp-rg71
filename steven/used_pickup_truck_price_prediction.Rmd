...
have high levels of collinearity. In addition, considering the large number of
predictors, we further conclude that this "full" model is relatively complex.

Despite the collinear relationships between the added predictors, the "full" model
...
### ii. Collinearity & Outlier Analysis
...
indicate collinearity, allows for a first round of model optimization/simplification
by removing redundant (collinear) predictors. The scatterplot matrix further
...
After analyzing collinearity and variable distribution we focus in on the observations,
...
```{r Collinearity Analysis pt. 1, echo=FALSE}
...
- `year` is nearly perfectly linear, as well as (`x + x^2`) quadratic, to our response, `price`
- `daysonmarket` is, to our surprise, not useful and adds lots of noise

&nbsp;

```{r Collinearity Analysis pt. 2, echo=FALSE}
...
- `power_hp` vs `torque_lb_ft` appear collinear
...
```{r Collinearity Analysis pt. 3, echo=FALSE}
...
factors (`vif`) to determine which predictors are collinear with 1 or more of the
...
(>5) indicates collinearity.

```{r Collinearity Analysis pt. 4, echo=FALSE, warning=FALSE}
v = vif(full_model)
v[v > 5]
```
...
- `height`, `length`, and `owner_count` each have particularly high `vif`
  - We'd like to better understand with which predictors these are collinear

- Multiple `regions` and `states` have a high `vif`
  - This will become significant, as we seek to build a global prediction model

```{r Collinearity Analysis pt. 5, echo=TRUE, message=FALSE, warning=FALSE}
...
We found it safe to conclude that `state` is collinear with `region`, and that
individual factors within the `state` variable are collinear with each other.

Removing the `state` predictor results in `region` having a low `vif`. This outcome
further supports our conclusions on their collinearity.

```{r Collinearity Analysis pt. 6, include=FALSE}
...
```{r Outlier Analysis pt. 2, echo=TRUE, warning=FALSE}
# analyze outlier observations to see if there are commonalities in their data
outlier_rows_to_remove = intersection_of_high_inf_high_outlier 
outlier_df = truck_df[outlier_rows_to_remove,]

# extraordinarily large number of Jeep Gladiators found in the outliers
nrow(outlier_df[outlier_df[,"model_name"] == c("gladiator", "Gladiator"), ]) /
    nrow(outlier_df) * 100

# several Ford F150 as well
nrow(outlier_df[outlier_df[,"model_name"] == c("F-150", "F150"), ]) /
    nrow(outlier_df) * 100

# a notably high count of 4x4 trucks
...
##### Examples of Clear Outliers Within Predictor Distributions
...
```{r Outlier Analysis pt. 4, echo=TRUE}
...
```{r Remove Outliers from Dataset, echo=TRUE}
truck_df = truck_df[-outlier_rows_to_remove,]
```

&nbsp;

```{r Outlier Analysis pt. 5, include=FALSE}
...
interaction model by highlighting its most significant interactions. These
interactions were selected through evaluating their `p-values` using `t` tests.
This approach resulted in a significant decrease in the number of predictors at
...
plot_fitted_resid(improved_mid_atlantic_int_model, main = "Improved Int. Model Fitted vs Residuals")
...
being collinear, but may not necessarily be collinear, depending on the region(s)
and state(s) in question.

Some further analysis regarding which states are truly/particularly significant
and which are collinear may have allowed us the ability to better process our
